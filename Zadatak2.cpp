...
void Pregled::PomjeriDanUnaprijed() {
    int broj_dana[]{31,28,31,30,31,30,31,31,30,31,30,31};
    int a[]{
        std::get<0>(this->dat.Ocitaj()),std::get<1>(this->dat.Ocitaj()),std::get<2>(this->dat.Ocitaj())
    };
    a[0]++;
...
public:
    ...
    Pregledi(std::initializer_list<Pregled> spisak_pregleda) : max(spisak_pregleda.size()), broj(spisak_pregleda.size()), elementi(new Pregled*[spisak_pregleda.size()]) {
...
void Pregledi::IspisiPregledeNaDatum(const Datum &datum) const {
    ...
    std::for_each(elementi,elementi+broj,[&a,datum](Pregled *p) {
        if(std::get<0>(datum.Ocitaj())==std::get<0>((p->DajDatumPregleda()).Ocitaj()) && 
           ...
           std::get<2>(datum.Ocitaj())==std::get<2>((p->DajDatumPregleda()).Ocitaj())) {
               a.push_back(*p);
           }
    });
     ...
     try {
         ...
         for(;;) {
             ...
             if(opcija==3) {
                 ...
                 a.ObrisiPregledePacijenta(ime_pacijenta);
